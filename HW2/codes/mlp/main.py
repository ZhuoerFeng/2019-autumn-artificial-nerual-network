...
with tf.Session() as sess:
    ...
    if FLAGS.is_train:
        ...
        for epoch in range(FLAGS.num_epochs):
            ...
            record_tr_acc.append(train_acc)
            record_tr_loss.append(train_loss)
            record_val_acc.append(val_acc)
            record_val_loss.append(val_loss)
            record_ts_acc.append(test_acc)
            record_ts_loss.append(test_loss)
...
filename = './data/B+1D+mlp.txt'

with open(filename, 'w', encoding="utf-8") as f:
    f.write(str(record_tr_acc) + "\n")
    f.write(str(record_tr_loss) + "\n")
    f.write(str(record_val_acc) + "\n")
    f.write(str(record_val_loss) + "\n")
    f.write(str(record_ts_acc) + "\n")
    f.write(str(record_ts_loss) + "\n")